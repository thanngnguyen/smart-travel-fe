...
export default function AdminTourEditPage() {
  ...
  const [activeDepartures, setActiveDepartures] = useState("");
  ...
  useEffect(() => {
    ...
    setActiveDepartures(tour.activeDepartures);
    setImageUrl(tour.imageUrl);
    setImageAlt(tour.imageAlt);
    setAssignedGuide(guideOptions[0]?.value ?? "");
    setSelectedDays(
      recurringDays.filter((day) => day.isSelected).map((day) => day.id),
    );
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <label className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(event) => setActiveDepartures(event.target.value)}
                  className="w-full rounded-xl border border-outline-variant/30 bg-surface-container-lowest px-3 py-2 text-sm font-semibold text-on-surface outline-none focus:border-primary"
                />
              </label>
              ...
              <label className="space-y-1.5 md:col-span-2">
                ...
                <select
                  value={assignedGuide}
                  onChange={(event) => setAssignedGuide(event.target.value)}
                  className="w-full rounded-xl border border-outline-variant/30 bg-surface-container-lowest px-3 py-2 text-sm font-semibold text-on-surface outline-none focus:border-primary"
                >
