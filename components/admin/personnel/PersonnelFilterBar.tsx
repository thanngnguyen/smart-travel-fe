...
import { PersonnelFilterOption } from "@/types/admin-personnel";

interface PersonnelFilterBarProps {
  searchQuery: string;
  roleFilters: PersonnelFilterOption[];
  statusFilters: PersonnelFilterOption[];
...
export default function PersonnelFilterBar({
  ...
  const activeRoleFilter =
    roleFilters.find((filter) => filter.isActive)?.id ??
    roleFilters[0]?.id ??
    "all";
  const activeStatusFilter =
    ...
    statusFilters[0]?.id ??
    "all";
  const roleOptions = roleFilters.map((filter) => ({
    value: filter.id,
    label: filter.label,
  }));
      ...
      <div className="flex flex-col xl:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Tìm theo tên nhân sự, email, mã nhân viên..."
